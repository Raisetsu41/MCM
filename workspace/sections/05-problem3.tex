...
模型设定上，延续问题二的常弹性（幂律）需求结构 \cite{tellis1988,hoch1995}，单品 $i$ 在售价 $p$ 下的日需求为
\begin{equation}
...
候选集构成与品类弹性见表\ref{tab:q3_candidates}。单品弹性诊断：45 个候选的有效样本量 $N_i$ 为 28~559；13 个单品原始弹性为正，其中 12 个借用品类弹性；选中单品收缩弹性区间 $[-2.52,-0.03]$，均值 -0.44。
...
\ref{fig:q3_price_qty} 展示售价—订购量散点。
...
\textbf{局限与展望：}单品需求独立假设忽略了替代效应\cite{kok2007,vanryzin1999}；确定性需求可升级为单品级报童\cite{federgruen1999,petruzzi1999}；净销量低估真实需求\cite{agrawal1996}；未按周末偏置上调基准需求，结果偏保守；满足率为软惩罚，可改为硬约束或鲁棒选品\cite{vanryzin1999,rusmevichientong2012}；未考虑动态折扣\cite{elmaghraby2003}与品类角色差异\cite{dhar2001}。