...
> **[cjg]** 第 1~7 条已实现：results/q2_sensitivity.csv（批发价/弹性/
> 损耗率/价格口径共 8 个情景）、q2_nv_det.csv（报童 vs 确定性）、
> q2_backtest_2022.csv（2022-07 同期回测）、q2_joint_sim.csv（品类相关
> 联合模拟）、q2_robustness_summary.json（校验契约）。关键发现：中位价
> 口径下水生根茎类、食用菌弹性符号翻转为正、利润虚高 +72.8%，该口径不可
> 用，固定权重主口径成立；2022-07 同期回测 MAPE 23.9%~66.4%，跨年预测
> 稳定性有限，作为局限写入论文。
